                                    ...
                                    <tr>
                                        ...
                                        <td align="center">
                                            @if ($item->expediente_tipo_seguimiento_estado == 1)
                                                <span style="cursor: pointer;" wire:click="cargarAlertaEstado({{ $item->id_expediente_tipo_seguimiento }})" class="badge text-bg-success text-light hover-elevate-down fs-6 px-3 py-2">Activo</span>
                                            @else
                                                <span style="cursor: pointer;" wire:click="cargarAlertaEstado({{ $item->id_expediente_tipo_seguimiento }})" class="badge text-bg-danger text-light hover-elevate-down fs-6 px-3 py-2">Inactivo</span></span>
                                            @endif
                                        </td>
                                        <td align="center">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select @error('tipo_seguimiento') is-invalid  @enderror" wire:model="tipo_seguimiento">
                                    ...
                                    @foreach ($tipoSeguimientoModel as $item)
                                        @php
                                            $isAssigned = $validarTipoSeguimiento->contains('id_tipo_seguimiento', $item->id_tipo_seguimiento);
                                        @endphp
                                        @if($item->tipo_seguimiento_estado == 1)
                                            <option value="{{ $item->id_tipo_seguimiento }}" {{ $isAssigned ? 'disabled' : '' }} class="{{$isAssigned ? 'text-gray-400' : ''}}">{{ $item->tipo_seguimiento }}</option>
                                        @endif
                                    @endforeach
                                </select>
